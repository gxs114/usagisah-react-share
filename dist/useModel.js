...
import { compose, isBoolean, isUndefined } from "./helpers";
/**
 * @description 双向数据绑定
 * @params
 ...
 * <Input {...bindValue} />
 */
export function useModel({ value, filters = [], skipError }) {
    ...
    function handleChange(v) {
        ...
        if (!isUndefined(v?.target?.value)) {
            newValue = v.target.value = compose(...filters)(v.target.value);
        }
        else if (!isUndefined(v)) {
            newValue = compose(...filters)(v);
        }
        else if (!skipError) {
...
export function useModelChecked(checked) {
